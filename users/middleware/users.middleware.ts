...
class UsersMiddleware {
    ...
    async validateRequiredUserBodyFields(req: express.Request, res: express.Response,
    ...
    async validateSameEmailDoesntExist(req: express.Request, res: express.Response, 
        next: express.NextFunction) {
    ...
    async validateSameEmailBelongToSameUser(req: express.Request, res: express.Response,
        next: express.NextFunction) {
        const user = await usersService.getUserByEmail(req.body.email);
        log(user);
    ...
    async validateUserExists(req: express.Request, res: express.Response, next: express.NextFunction) {
        ...
        if (user) {
            ...
            res.status(400).send({error: `User ${req.params.userId}`});
        }
    }
